...
export const getRefreshTokenRepo = async (refreshTokenHash: string): Promise<Pick<refreshTokenRow, 'is_revoked' | 'user_id' | 'expires_at'> | null> => {
    const result = await pool.query<Pick<refreshTokenRow, 'is_revoked' | 'user_id' | 'expires_at'>>(`SELECT is_revoked, user_id ,expires_at FROM refreshtoken  WHERE refresh_hash=$1`, [refreshTokenHash]);
    return result.rows[0] || null;
}
export const revokeAllRefreshTokens = async (user_id: string): Promise<tokenRevocationionResult | null> => {
...
export const revokeToken = async (oldToken: string): Promise<tokenRevocationionResult | null> => {
    const result = await pool.query<tokenRevocationionResult>(`UPDATE refreshtoken SET is_revoked = $1 , revoked_at=NOW() WHERE refresh_hash=$2 RETURNING revoked_at`, [true, oldToken]);
    return result.rows[0] || null;
}